...
SECRET_PATTERNS = {
    ...
    "GitHub Token": (r"ghp_[0-9a-zA-Z]{36}|gho_[0-9a-zA-Z]{36}|ghs_[0-9a-zA-Z]{36}", "critical",
    ...
    "JWT Token": (r"eyJ[A-Za-z0-9_/+\-]{10,}={0,2}\.[A-Za-z0-9_/+\-]{10,}={0,2}\.[A-Za-z0-9_/+\-]{10,}={0,2}", "medium",
    ...
    "Stripe Publishable Key": (r"pk_live_[0-9a-zA-Z]{24}", "low",
...
class TrufflehogTool(SimpleToolRunner):
    ...
    async def run(self, url_list_file: str) -> AsyncIterator[ToolEvent | Finding]:
        ...
        args = [
            "filesystem", "--directory", "/dev/stdin",
            "--json", "--no-update",
        ]
...
class GitleaksTool(SimpleToolRunner):
    ...
    async def run_on_content(self, content: str, source_url: str) -> AsyncIterator[ToolEvent | Finding]:
...
async def scan_js_content_regex(js_url: str, content: str) -> list[Finding]:
    ...
    for pattern_name, (pattern, severity, remediation) in SECRET_PATTERNS.items():
        ...
        for match in matches:
            ...
            if any(fp in evidence.lower() for fp in ["your_", "xxx", "placeholder", "example", "dummy", "changeme"]):
                continue
...
def _parse_trufflehog_line(line: str) -> Finding | None:
    ...
    source_url = data.get("SourceMetadata", {}).get("Data", {}).get("Git", {}).get("file", "")
    if not source_url:
