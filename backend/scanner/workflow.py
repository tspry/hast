...
async def start_scan(
    ...
    for sid, task in list(_active_scans.items()):
        if not task.done():
            await emit("scan_error", {"error": f"Scan {sid} is already running. Stop it first."})
            return sid

    _stop_flags[scan_id] = False
    task = asyncio.create_task(
        _run_scan(scan_id, target, profile, emit, resume)
    )
...
async def _run_scan(
    ...
    try:
        ...
        recon_result = {"waf_detected": False, "waf_name": None,
                        "open_ports": [], "technologies": [], "findings": []}
        discovery_result = {"urls": [target], "js_urls": []}
        scan_findings = []
        ...
        if not (resume and checkpoints.get("recon") == "completed"):
            ...
            try:
                ...
                await db.save_checkpoint(scan_id, "recon", "completed", {
                ...
                await emit("log", {"tool": "scanner", "stream": "error",
        ...
        if profile != "quick":
            if not (resume and checkpoints.get("discovery") == "completed"):
                ...
                try:
                    discovery_result = await run_discovery(target, profile, emit, scan_id)
                    await db.insert_urls(scan_id, discovery_result["urls"], "crawler", False)
                    await db.insert_urls(scan_id, discovery_result["js_urls"], "crawler", True)
                    await db.save_checkpoint(scan_id, "discovery", "completed", {
        ...
        if not (resume and checkpoints.get("scanning") == "completed"):
            ...
            try:
                scan_findings = await run_scanning(
                    target=target,
                ...
                await db.save_checkpoint(scan_id, "scanning", "completed",
...
async def _handle_stop(scan_id: str, emit: Callable) -> None:
    await db.update_scan(scan_id, status="stopped",
                         finished_at=datetime.now(timezone.utc).isoformat())
