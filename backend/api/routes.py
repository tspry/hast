...
from backend.scanner.workflow import is_scan_running, start_scan, stop_scan
...
async def get_findings(
    ...
    if keyword:
        ...
        findings = [
            ...
            or kw in f["url"].lower()
            or kw in f["evidence"].lower()
        ]
...
async def get_scan_diff(scan_id: str):
    ...
    resolved = [
        f for f in prev_findings if f"{f['url']}|{f['name']}" not in current_keys
    ]
...
async def export_json(scan_id: str):
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=hast_{scan_id[:8]}.json"
        },
    )
...
async def export_pdf(scan_id: str):
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=hast_report_{scan_id[:8]}.pdf"
        },
    )
...
def _generate_pdf(scan: dict, findings: list[dict]) -> bytes:
    ...
    elements.append(
        Paragraph(f"<b>Target:</b> {scan.get('target', 'N/A')}", meta_style)
    )
    elements.append(
        Paragraph(f"<b>Profile:</b> {scan.get('profile', 'N/A')}", meta_style)
    )
    elements.append(
        Paragraph(f"<b>Scan Date:</b> {scan.get('started_at', 'N/A')}", meta_style)
    )
    elements.append(
        Paragraph(f"<b>Status:</b> {scan.get('status', 'N/A')}", meta_style)
    )
    ...
    for f in findings[:200]:  # cap at 200 for PDF
        ...
        if f.get("evidence"):
            ev_text = (
                f.get("evidence", "")[:300].replace("<", "&lt;").replace(">", "&gt;")
            )
...
async def bulk_scan_summary(
    targets: str = Query(..., description="Comma-separated list of targets")
):
    ...
    for target in target_list:
        ...
        match = next(
            (
                s
                for s in all_scans
                if s["target"] == target and s["status"] == "completed"
            ),
            None,
        )
        if not match:
...
async def tools_status():
    ...
    return {
        "tools": {
            t: {
                "available": bool(resolve_tool_path(t)),
                "path": resolve_tool_path(t) or "not found",
            }
            for t in tools
        }
    }
